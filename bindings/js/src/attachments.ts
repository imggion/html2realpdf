...
export function preparePdfExtras(options: Pick<RenderOptions, "conformance" | "attachments">): PdfExtras {
  if (options.conformance !== undefined && options.conformance !== "pdfa-3u") fail("Unsupported PDF conformance");
  ...
  result.attachments = options.attachments.map((attachment): PreparedAttachment => {
    if (!attachment || typeof attachment.name !== "string" || !attachment.name.trim()) fail("Attachment name must not be empty");
    ...
    if (typeof mimeType !== "string" || !/^[!#$%&'*+.^_`|~0-9A-Za-z-]+\/[!#$%&'*+.^_`|~0-9A-Za-z-]+$/.test(mimeType)) fail("Invalid attachment MIME type");
    const relationship = attachment.relationship === undefined ? "Unspecified" : attachment.relationship;
    if (!["Source", "Data", "Alternative", "Supplement", "Unspecified"].includes(relationship)) fail("Invalid attachment relationship");
    if (!(attachment.data instanceof ArrayBuffer) && !(attachment.data instanceof Uint8Array)) fail("Attachment data must be an ArrayBuffer or Uint8Array");
    let data: Uint8Array<ArrayBuffer>;
    try {
      data = new Uint8Array(attachment.data instanceof Uint8Array ? attachment.data : new Uint8Array(attachment.data));
    } catch { return fail("Attachment buffer is detached or invalid"); }
    ...
    if (attachment.modifiedAt !== undefined) {
      ...
      if (!(date instanceof Date) || !Number.isFinite(date.getTime()) || date.getUTCFullYear() < 1 || date.getUTCFullYear() > 9999) fail("Invalid attachment modification date");
