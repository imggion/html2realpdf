...
- `.github/workflows/ci.yml` runs the complete release gate for pull requests into `main` and pushes to `main`; successful pushes retain a commit-addressed npm tarball. `.github/workflows/publish-npm.yml` is manual-only and publishes that validated artifact when the latest SemVer tag matches `main` HEAD. Tag pushes do not trigger either workflow.
