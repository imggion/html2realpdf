...
- Early-stage Zig package named `html2realpdf`; current implemented behavior is an HTML tokenizer plus a tolerant DOM tree builder, with native CLI and WebAssembly entrypoints.
...
- `build.zig` defines the native executable from `src/main.zig`, the package/root module from `src/root.zig`, and the wasm executable from `src/wasm.zig`.
...
- `make test-debug` runs both debug dump targets.
